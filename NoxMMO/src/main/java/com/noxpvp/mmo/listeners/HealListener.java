...
public class HealListener extends NoxListener<NoxMMO> {
	...
	public void onHeal(EntityRegainHealthEvent event) {
		...
		if (!(event.getEntity() instanceof LivingEntity)
		        || (healed = (LivingEntity) event.getEntity()) == null)
			return;
		...
		if (getPlugin().getMMOConfig().get("effect.heal.heart", Boolean.class,
		        Boolean.TRUE)) {
			StaticEffects.HeartEffect(healed, amount, getPlugin());
		}
		
		if (getPlugin().getMMOConfig().get("effect.heal.heal-amount-particle",
		...
		if (healed instanceof Player
			...
			for (int i = 0; i < amount; i++) {
				Bukkit.getScheduler().runTaskLater(getPlugin(), new Runnable() {
					...
					public void run() {
						StaticEffects.playSound((Player) healed,
						        PacketSounds.LiquidLavaPop, 2, 3);
					}
				}, i);
			}
	}
