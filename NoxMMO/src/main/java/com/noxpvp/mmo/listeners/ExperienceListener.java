...
public class ExperienceListener extends NoxListener<NoxMMO> {
	...
	public ExperienceListener(NoxMMO mmo) {
		...
		if (this.cachedMultipliers == null)
			this.cachedMultipliers = new HashMap<String, Integer>();
	}
	...
	public int getExpBoost(Player player) {
		if (VaultAdapter.permission.has(player, NoxMMO.PERM_NODE + ".exp.booster.5"))
			return 4;
		else if (VaultAdapter.permission.has(player, NoxMMO.PERM_NODE
		        + ".exp.booster.4"))
			return 3;
		else if (VaultAdapter.permission.has(player, NoxMMO.PERM_NODE
		        + ".exp.booster.3"))
	...
	public void onKill(EntityDeathEvent event) {
		...
		EntityDamageByEntityEvent last = (EntityDamageByEntityEvent) ((killed
		        .getLastDamageCause() instanceof EntityDamageByEntityEvent) ? killed
		        .getLastDamageCause() : null);
		if (last == null)
			return;
		
		Player attacker = (Player) (last.getDamager() instanceof Player ? last
		        .getDamager() : null);
		if (attacker == null)
			return;
	...
	public void onBreak(BlockBreakEvent event) {
		...
		if (mine.contains(event.getBlock().getType()))
			...
			player.addExp(ExperienceType.EXCAVATION, MathUtil.floor(excavationExp
			        * xpMulti));
	}
	...
	public void onTame(EntityTameEvent event) {
		...
		MMOPlayer mmoPlayer;
		if ((mmoPlayer = MMOPlayerManager.getInstance().getPlayer(player)) == null)
			return;
